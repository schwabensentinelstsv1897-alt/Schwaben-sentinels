...
| v2.0.6 | Mar 2026 | Install as app guide added to Help tab (iOS + Android steps) |
| v2.0.7 | Mar 2026 | Firebase Anonymous Auth, first-come-first-served squad system, duplicate Help tab fixed |
| v2.0.8 | Mar 2026 | Match attendance (played/no-show/walk-in), help logo fix, bug report fixes |
| v2.0.9 | Mar 2026 | Emoji picker grid, activity monthly archive + pagination, Add Result fix, delete match fix |
| v2.1.0 | Mar 2026 | Soft delete for players and matches, Archive section in Admin with restore and permanent delete |
...
## 32. v2.0.7 — Anonymous Auth, Squad System & Bug Fixes

**Date:** March 2026

### Firebase Anonymous Authentication
App now signs in anonymously via Firebase Auth before any database reads/writes.

**Changes:**
- Added `firebase-auth.js` SDK import
- `signInAnonymously(auth)` called during Firebase init
- `firebaseReady` event only fires after auth token is obtained
- Falls back gracefully if auth fails (app still works, just unprotected)
- Console shows: `Firebase Auth: anonymous sign-in OK ✅`

**Firebase Rules (update after deploying v2.0.7):**
```json
{
  "rules": {
    "ss2026": {
...
```
This stops Firebase security warning emails permanently.

### First-Come-First-Served Squad System

Players who click **Available** are ranked by `lastEdit` timestamp ascending. First N players get confirmed spots, the rest go on a waitlist.

**Logic (`getSquadPositions(league, idx)`):**
- Only `status === 'yes'` counts — Maybe and No are excluded
- Players sorted by `lastEdit` ascending (earliest = highest priority)
- First `squadSize` players → confirmed `#1 #2 ... #14`
- Remaining players → waitlisted `W1 W2 W3...`
- If a confirmed player changes to Maybe or Unavailable → loses spot instantly, W1 promotes

**UI changes:**
- Match card availability buttons: green `#3` or amber `W1` badge next to buttons
- Squad progress bar: `X/14 confirmed · N spots left · N waiting`
- Team overview list: position badge shown next to each player's name
- Admin → Settings → **Squad Size** field (default 14, configurable 1–20)

**Settings:**
- `sharedData.settings.squadSize` — stored in Firebase with other settings
- `getSquadSize()` helper returns current value or 14 as default

### Duplicate Help Tab Fix
Help tab was added twice — once in v2.0.3 and again during the v2.0.5 More drawer update. Second duplicate removed. Tab order: 50 Overs · T20 · Friendly · Practice · Overview · Activity · Matches · Team · Photos · ❓ Help · 🐛 Bugs · ⚙️ Admin


---

## 33. v2.0.9 — Emoji Picker, Activity Archive, Bug Fixes

**Date:** March 2026

### Emoji Picker for Custom Badges/Improvements/Roles
Replaced emoji text input fields (`new-ach-emoji`, `new-imp-emoji`, `nr-emoji`) with a visual grid picker.

**Implementation:**
- `openEmojiPicker(inputId)` — opens modal grid, stores target input ID in `_epid`
- `selectEmoji(em)` — sets hidden input value and updates preview button text, closes modal
- Click outside modal closes it
- Uses `data-eid` attribute on button to avoid quote escaping issues in JS strings

**100 emoji across 5 groups:**
- Cricket: 🏏🎳🧤🥊🎯🏃🏅🥇🏆⚡ + 10 more
- Performance: 📈💪🚀💎🦁🐉🦅⚔️🛡️🧠 + 10 more
- Team: 🤜🙌👏🫂🧩🔗📣📢🎺🎸 + 10 more
- Fun: 😤💯🔑🗝️⚙️🔩🪄🎲🎰🃏 + 10 more
- Nature: 🌊⛰️🌋🌪️❄️☀️🌙💧🌿🍃 + 10 more

### Add Result Button Fixed
`showMatchResultForm()` was crashing because `existingAtt` variable was referenced in the attendance grid HTML but never defined in the function scope. Fixed by adding `var existingAtt = (sharedData.matchAttendance && sharedData.matchAttendance[matchId]) || {};` at the top of the function.

### Delete Match Visibility Fixed
Delete Match button now shows for any player listed in `permissions.admins`, not just when the admin panel PIN (`adminUnlocked`) is active. Check: `adminUnlocked || permissions.admins.indexOf(currentPlayer) >= 0`.

### Bug Report Type Label
`renderMyBugReports()` now shows emoji + label text together (e.g. `🐛 Bug`, `🎨 UI Issue`, `💡 Feature Request`) instead of just the emoji icon.

### Activity Log — Monthly Archive + Pagination
Complete rewrite of `renderActivity()`:

**Month timeline:** Buttons for each month that has data (YYYY-MM), sorted newest first. Stored in `window._actMonth`. Clicking switches the view.

**Pagination:** 20 entries per page. Load More button shows remaining count. Page state stored in `window['_actPg_'+month]`.

**New helper functions:**
- `setActivityMonth(m)` — switches to month m, resets page to 1
- `loadMoreAct(m)` — increments page for month m

Added `<div id="act-month-bar">` to activity panel HTML as the timeline injection target.

---

## 34. v2.1.0 — Soft Delete & Archive System

**Date:** March 2026

### Overview
Deletion of players and matches is now non-destructive. Items are moved to an archive in Firebase rather than permanently erased. Admin can restore or permanently delete from the Archive section.

### Firebase Data Structure
```
archive/
  players/{safeKey}: {
    name, playerData, pin, profile, tempPin,
    deletedAt, deletedBy
  }
  matches/{matchId}: {
    data, league, scheduleKey,
    deletedAt, deletedBy
  }
```
`safeKey` = player name with `.#$[]` replaced by `_` (Firebase key constraint).

### Match Soft Delete (`deleteMatchConfirm`)
- Confirmation dialog explains item will be archived
- Full match data copied to `archive/matches/{matchId}`
- Match removed from `schedule/{key}/{matchId}`
- Written to Firebase immediately

### Player Soft Delete (`deletePlayer`)
- Collects: playerData, pin hash, profile, tempPinDisplay
- Copies to `archive/players/{safeKey}`
- Removes from: players, pins, tempPins, tempPinDisplay, profiles, permissions arrays
- Written to Firebase immediately

### Archive Section in Admin Panel
Located after Role Assignment section. Two tabs:

**👥 Players tab:**
- Lists archived players with name, deleted by, deleted at
- ↩ Restore — calls `restorePlayer(safeKey)`:
  - Checks player doesn't already exist
  - Restores playerData, pin, profile to live Firebase nodes
  - Removes from archive
  - Calls `renderAdminPlayers()` and `renderArchive()`
- ✕ Delete — calls `permanentDeletePlayer(safeKey)` with double confirmation

**📋 Matches tab:**
- Lists archived matches with teams, date, league, deleted by/at
- ↩ Restore — calls `restoreMatch(matchId, scheduleKey)`:
  - Writes match data back to `schedule/{scheduleKey}/{matchId}`
  - Removes from archive
  - Calls `renderMatchesTab()` and `renderArchive()`
- ✕ Delete — calls `permanentDeleteMatch(matchId)` with double confirmation

### Archive renders on Admin panel open
`renderArchive()` added to the admin panel init chain alongside `renderAdminBugReports()`.
